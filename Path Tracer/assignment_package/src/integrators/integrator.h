...
public:
    Integrator(Bounds2i bounds, Scene* s, std::shared_ptr<Sampler> sampler, int recursionLimit)
        : scene(s), camera(&(s->camera)), thinlenscamera(std::make_shared<ThinLensCamera>(s->thinLensCamera)),
          film(&(s->film)), sampler(sampler), bounds(bounds), recursionLimit(recursionLimit)
    {
        ClampBounds();
    }
...
protected:
    ...
    std::shared_ptr<ThinLensCamera> thinlenscamera;
