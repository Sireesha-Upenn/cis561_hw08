...
inline bool IsBlack(const Color3f& c) { return (c.r == 0.f && c.g == 0.f && c.b == 0.f); }

inline float AbsDot(const Vector3f& a, const Vector3f& b) { return glm::abs(glm::dot(a, b)); }

inline bool SameHemisphere(const Vector3f &w, const Vector3f &wp) { return w.z * wp.z > 0; }

inline Normal3f Faceforward(const Normal3f &n, const Vector3f &v) { return (glm::dot(n, v) < 0.f) ? -n : n; }
...
//All Phi functions
inline float CosPhi(const Vector3f &w) { return (SinTheta(w) == 0) ? 1 : glm::clamp(w.x / SinTheta(w), -1.f, 1.f); }

inline float SinPhi(const Vector3f &w) { return (SinTheta(w) == 0) ? 0 : glm::clamp(w.y / SinTheta(w), -1.f, 1.f); }

inline float Cos2Phi(const Vector3f &w) { return CosPhi(w) * CosPhi(w); }

inline float Sin2Phi(const Vector3f &w) { return SinPhi(w) * SinPhi(w); }

//Called from specularBTDF
