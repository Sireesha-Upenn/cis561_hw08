...
void JSONReader::LoadSceneFromFile(QFile &file, const QStringRef &local_path, Scene &scene)
{
    if(file.open(QIODevice::ReadOnly))
    {
        ...
        QJsonObject sceneObj, camera, thinlenscamera;
        ...
        foreach(const QJsonValue &frame, frames) {
            ...
            else if(sceneObj.contains(QString("thinlenscamera"))) {
                thinlenscamera = sceneObj["thinlenscamera"].toObject();
                scene.SetThinLensCamera(LoadThinLensCamera(thinlenscamera));
                scene.SetCamera(LoadCamera(thinlenscamera));
            }
...
bool JSONReader::LoadGeometry(QJsonObject &geometry, QMap<QString, std::shared_ptr<Material>> mtl_map,
                              const QStringRef &local_path, QList<std::shared_ptr<Primitive>> *primitives)
{
...
bool JSONReader::LoadLights(QJsonObject &geometry, QMap<QString, std::shared_ptr<Material>> mtl_map, const QStringRef &local_path, QList<std::shared_ptr<Primitive>> *primitives, QList<std::shared_ptr<Light>> *lights)
{
    ...
    else if(QString::compare(type, QString("Spot")) == 0)
    {
    ...
    //Added Code for SpotLights and PointLights
    else if (QString::compare(lgtType, QString("SpotLight")) == 0)
    {
        ...
        lightType = std::make_shared<SpotLight>(LoadTransform(transform), lightColor * intensity , totalWidth, falloff);
    }
    else if (QString::compare(lgtType, QString("PointLight")) == 0)
    {
        ...
        Float intensity = static_cast< float >(geometry["intensity"].toDouble());
        QJsonObject transform = geometry["transform"].toObject();
        lightType = std::make_shared<PointLight>(LoadTransform(transform), lightColor * intensity);
    }
    else
    {
        std::cout << "Could not parse the geometry!" << std::endl;
...
Camera JSONReader::LoadCamera(QJsonObject& camera)
{
    ...
    if(camera.contains(QString("focalDistance"))) {}
    result.RecomputeAttributes();
    return result;
}

ThinLensCamera JSONReader::LoadThinLensCamera(QJsonObject& tlCamera)
{
    ...
    if(tlCamera.contains(QString("lensRadius"))) result.lensRadius = tlCamera["lensRadius"].toDouble();
    if(tlCamera.contains(QString("focalDistance"))) result.focalDistance = tlCamera["focalDistance"].toDouble();
    result.RecomputeAttributes();
    return result;
}
