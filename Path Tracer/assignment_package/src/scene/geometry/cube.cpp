...
{
    //Area of a cuboid
    Float l = transform.getScale().x, w = transform.getScale().y, h = transform.getScale().z;
...
bool Cube::bothIntersects(const Ray &r, CSGIntersection& csg_isect) const
{
    ...
    if(t_n < t_f){
        ...
        //Setting both t values for intersection
        csg_isect.t1 = t_n;
        csg_isect.t2 = t_f;
