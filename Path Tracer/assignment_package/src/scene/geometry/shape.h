...
public:
    ...
    //Added for CSG Intersection
    //Make the child class override this function of set both t's of an intersection
    virtual bool bothIntersects(const Ray &ray, CSGIntersection& csg_isect) const;

    float Pdf(const Intersection &ref, const Vector3f &wi);
