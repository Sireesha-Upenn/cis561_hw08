...
{
    //DELETEME
    return 4.f * Pi * transform.getScale().x * transform.getScale().y; // We're assuming uniform scale
}
...
bool Sphere::bothIntersects(const Ray &ray, CSGIntersection& csg_isect) const
{
    ...
    if(t >= 0)
    {
        Point3f P = glm::vec3(r_loc.origin + t*r_loc.direction);
        InitializeIntersection(isect, t, P);
        csg_isect.isect = isect;
        csg_isect.t1 = (-B - sqrt(discriminant))/(2*A);
        csg_isect.t2 = (-B + sqrt(discriminant))/(2*A);
        return true;
    }
    return false;
}
